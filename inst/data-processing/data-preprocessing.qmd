...
#| echo: false
cfg <- config::get()
sitrep_sources_metadata <- file.path(cfg$pins_username, cfg$metadata_name_template, fsep = "/")
    ...
    agent_info = purrr::map(
      ...
      ~ create_informant(agent = .x) |>
        ...
        info_snippet("n_regions",
                     ~ . %>% # nolint: pipe_consistency_linter
                       dplyr::pull("NHS England Region") %>% # nolint: pipe_consistency_linter
                       unique() %>% # nolint: pipe_consistency_linter
                       length()) |>
    ...
    data_type_check = purrr::map(
      ...
      ~ .x |>
        ...
        rows_distinct(c("Code", "Name"),
                      preconditions = ~ . %>% # nolint: pipe_consistency_linter
                        tidyr::drop_na()) |>
    ...
    reformat_agent = purrr::map2(
      ...
      ~ create_agent(
        ...
        col_vals_make_set(
          columns = "date",
          set = .y$date,
          preconditions = ~ . %>% dplyr::filter(bed_type == "CC"), # nolint: pipe_consistency_linter
          segments = vars(code),
        ...
        col_vals_make_set(
          columns = "date",
          set = .y$date,
          preconditions = ~ . %>% dplyr::filter(bed_type == "G&A"), # nolint: pipe_consistency_linter
          segments = vars(code),
      ...
      tryCatch(
        {
          ...
          pin_name <- pin_write(
            board = board,
            x = data_reformat,
            name = file.path(cfg$pins_username,
                             sprintf(cfg$fulldata_name_template, year),
                             fsep = "/"),
            type = "parquet",
...
pin_name <- pin_write(
  ...
  x = final_metadata,
  name = file.path(cfg$pins_username,
                   sprintf(cfg$metadata_name_template, year),
                   fsep = "/"),
  type = "parquet",
  title = "NHS Flu Data Sources",
  description = paste("Metadata containing the last MD5 sums",
                      "for each of the previous year's worth of data.")
)
```
