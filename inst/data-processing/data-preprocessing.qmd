...
get_download_url <- function(year_end) {
  rvest::read_html(
    sprintf(
      "https://www.england.nhs.uk/statistics/statistical-work-areas/uec-sitrep/urgent-and-emergency-care-daily-situation-reports-%d-%d/",
      year_end - 1L,
      year_end - 2000L
    )
  ) |>
...
new_metadata <- dplyr::mutate(
  sources_metadata,
  temp_dest = dplyr::if_else(.data$to_download, tempfile(paste0("sitrep-", .data$year), fileext = ".xlsx"), NA_character_),
  download = dplyr::if_else(.data$to_download, download.file(url = .data$new_url, destfile = .data$temp_dest), NA_integer_),
  new_md5 = dplyr::if_else(.data$to_download, tools::md5sum(.data$temp_dest), NA_character_),
  ...
  dplyr::mutate(
    ...
    agent_info = purrr::map(
      ...
      ~ create_informant(agent = .x) |>
          ...
          info_snippet("n_regions", ~ . %>% dplyr::pull("NHS England Region") %>% unique() %>% length()) |>
  ...
  dplyr::mutate(
    data_type_check = purrr::map(
      ...
      ~ .x |>
          ...
          rows_distinct(c("Code", "Name"), preconditions = ~ . %>% tidyr::drop_na()) |>
    ...
    reformat_agent = purrr::map2(
      ...
      ~ create_agent(
          ...
          rows_distinct(
            ...
            brief = "For each trust, every date value does not appear more than once for the CC bed type."
          ) |>
          rows_distinct(
            columns = "date",
            preconditions = . %>% dplyr::filter(bed_type == "G&A"),
            ...
            brief = "For each trust, every date value does not appear more than once for the G&A bed type."
          ) |>
          interrogate()
    )
  )
...
all_uploaded <- purrr::pmap(
  ...
  function(year, data_reformat, reformat_agent, data_type_check) {
    ...
    if (all_passed(reformat_agent) && all_passed(data_type_check)) {
      ...
      cli::cli_alert_danger("Data was not exported because the data quality checks did not fully pass.")
    }
  }
)
